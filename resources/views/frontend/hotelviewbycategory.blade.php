<x-frontend-app-layout>
    ...
    <div class="wrapper">
        ...
        <section class="section">
            ...
            <p class="sp_pera">{{ $category->content }}</p>
            <div class="row maingap">
                @foreach ($hotels as $hotels)
                <div class="mb-4 col-xs-12 col-sm-12 col-md-4 col-lg-3">
                    <div class="card h-100 shadow-sm tripmainbox" style="border-radius: 20px; overflow: hidden;">
                        <img src="{{ asset($hotels->firstImage->file_url ?? 'images/default-trip.jpg') }}"
                            alt="{{ $hotels->name ?? 'Trip Image' }}"
                            class="card-img-top"
                            style="object-fit: cover;">
                        <div class="tripstpoic">{{ $hotels->name }}</div>
                        <div class="subjectbox">
                            {{ $hotels->specification }}
                        </div>
                        <div class="tripsmintro">{{ \Illuminate\Support\Str::limit($hotels->content, 100, '...') }}</div>
                        <div class="pricerequest-box">
                            Price on request
                        </div>
