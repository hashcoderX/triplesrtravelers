    ...
    <div class="row">
        ...
        <div class="invoice-container" style="width:100%; font-size:1em; display:flex;">
            <div class="invoice-to" style="width:50%;">
                <p style="line-height: 0.5px;">Invoice To - {{ $invoice->customername }}</p>

                <div class="contactcontainer" id="contactcontainer" style="position :absolute;margin-top:-110px;margin-left:70px;">
                    <p style="line-height: 0.5px;">Invoice # - {{ $invoice->id }}</p>
                    <p style="line-height: 0.5px;">Date - {{ $invoice->date }} </p>
                    <p style="line-height: 0.5px;">Ref- {{ $invoice->reference }} </p>
                    <p style="line-height: 0.5px;">Note - {{ $invoice->description }} </p>
                </div>
            </div>
        ...
        <div style="display: flex; justify-content: space-between; align-items: flex-start; padding-top: 10px;">
            ...
            <table>
                <tr>
                    <td>No of items - {{ $invoiceItems->count() }}</td>
                </tr>
                <tr>
                    <td>Total Amount - {{ number_format($invoice->total_bill, 2) }}</td>
                </tr>
                <tr>
                    <td>Discount - {{ number_format($invoice->discount, 2) }}</td>
                </tr>
                <tr>
                    <td>Net Amount - {{ number_format($invoice->net_total, 2) }}</td>
                </tr>
            </table>
        </div>
